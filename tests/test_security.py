...
def sign(url=URL, params=PARAMS, nonce=NONCE, token=TOKEN):
    base_url = construct_post_url(url, params)
    return get_signature_v3(token.encode(), base_url, nonce.encode()).decode()


def test_signature_matches_plivos_documented_construction():
    """Our fixture signature is generated by Plivo's production SDK."""
    assert sign()
...
def headers_for(params, path="/voice/turn", token=TOKEN):
    sig = sign(f"https://voice.test{path}", params, NONCE, token)
...
def test_multiple_active_tokens_any_match_accepted(configured):
    ...
    params = {"CallUUID": "abc"}
    good = sign("https://voice.test/voice/turn", params, NONCE, TOKEN)
