...
`--gateway-container` / `--chat-container`). It prompts for test caller numbers,
...
Test numbers are the phones callers dial FROM. Only new, transcribed streaming
turns from those numbers are recorded. Open the call in Chat Manager and use
Load audio beneath a customer message. WAV, original mu-law and diagnostics
